      ...
      if (!tripsError && tripsData) {
        const formattedTrips: Trip[] = tripsData.map(t => {
          ...
          parsedMembers = parsedMembers.map((m: any) => {
            ...
            if (u) {
              m.name = m.nickname || u.full_name || m.name || 'Traveler';
        ...
        if (!postsError && postsData) {
          formattedPosts = postsData.map((p: any) => {
            ...
            // Extract author from formattedTrips which is already enriched with userMap and nicknames
            let authorName = 'Traveler';
            let authorAvatar = undefined;
            const enrichedAuthor = formattedTrips.find(t => t.id === p['trip_id'])?.members?.find((m: any) => m.id === p['user_id']);
            
            if (enrichedAuthor) {
              authorName = enrichedAuthor.name || 'Traveler';
              authorAvatar = enrichedAuthor.avatar;
            } else {
              // Fallback to userMap if they left the trip
              const u = userMap.get(p['user_id']);
              if (u) {
                const isAutoGeneratedName = u.full_name === u.email?.split('@')[0];
                authorName = (!isAutoGeneratedName && u.full_name) ? u.full_name : (u.full_name || 'Traveler');
                authorAvatar = u.avatar_url;
              } else if (p.trips && p.trips.members) {
                // Absolute fallback to raw trips.members
